...
int main(int argc, char* argv[])
{
  ...
  if (!game_specified)
  {
    ...
    for (const auto& entry : std::filesystem::directory_iterator("."))
    {
      ...
      if (ext == ".iso" || ext == ".gcm" || ext == ".ciso" || ext == ".rvz")
      {
        game_path = entry.path().string();
        break;
      }
    }
    ...
    {
      boot = BootParameters::GenerateFromFile(
          game_path, BootSessionData(std::nullopt, DeleteSavestateAfterBoot::No));
  ...
  else
  {
    ...
    Config::SetCurrent(Config::MAIN_SKIP_IPL, false);
